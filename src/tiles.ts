...
// Format mirrors etl/_tile_io.py exactly.
...
export function packTileId(tx: number, ty: number): number {
  // Match etl/_tile_io.py: tx in high 16 bits, ty in low 16 bits, both as int16.
...
// Tile LRU budget. Workers isolate is 128 MB; the L1 highway overlay lives in
// the L1Router DurableObject now (see src/l1_router.ts), so this budget only
// has to leave headroom for request/heap + the raw-bytes LRU below.
