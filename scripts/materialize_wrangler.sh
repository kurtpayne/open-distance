...
KV_ID=$(wrangler kv namespace list 2>/dev/null \
  | python3 -c "import sys, json; arr=json.load(sys.stdin); print(next((k['id'] for k in arr if k.get('title') in ('HHAPI_CACHE','hhapi-HHAPI_CACHE')), ''))")
[[ -n "$KV_ID" ]] || { echo "ERROR: HHAPI_CACHE KV namespace not found. Run scripts/provision.sh first." >&2; exit 1; }
